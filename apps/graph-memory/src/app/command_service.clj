...
(def ^:private trace-entities?
  (boolean (some-> (System/getenv "TRACE_ENTITIES") str/trim not-empty)))
  ...
  (when trace-relations?
    (let [data (if (map? payload)
                 (-> payload
                     ...
                     (select-keys [:type :relation/type :src :dst :confidence :provenance]))
                 payload)]
      (println (format "[trace relation %s] %s" stage (pr-str data))))))

(defn- trace-entity [stage payload]
  (when trace-entities?
    (println (format "[trace entity %s] %s"
                     stage
                     (pr-str (select-keys payload [:id :name :type :external-id :source]))))))

;; -- Relation/Entity helpers -------------------------------------------------

(defn- arxana-store? [store]
  (and store (satisfies? arxana/ArxanaStore store)))

(defn- present-name [value]
  (when (and (string? value)
             (not (str/blank? (str value))))
    (str/trim (str value))))

(defn- entity-by-name [conn value]
  (when-let [lookup (present-name value)]
    (store/resolve-name->eid conn lookup)))

(defn- entity-by-external-id [conn external-id]
  (when-let [lookup (present-name external-id)]
    (try
      (store/fetch-entity conn {:external-id lookup} {})
      (catch Throwable _ nil))))

(defn- distinct-entities-by-id [entities]
  (reduce (fn [acc {:keys [id] :as entity}]
            (if (or (nil? id) (some #(= (:id %) id) acc))
              acc
              (conj acc entity)))
          []
          entities))

(defn- entity-by-id-string [conn value]
  (when value
    (let [lookup (str/trim (str value))]
      (try
        (store/fetch-entity conn {:id (UUID/fromString lookup)} {})
        (catch IllegalArgumentException _
          (store/fetch-entity conn {:name lookup} {}))
        (catch Throwable _ nil)))))

(defn- primary-entity [conn ident]
  (or (entity-by-name conn ident)
      (entity-by-external-id conn ident)
      (entity-by-id-string conn ident)))

(defn- related-entities [conn entity]
  (let [id-name (some-> (:id entity) str)
        name-alias (or (:entity/name entity) (:name entity))]
    (distinct-entities-by-id
     (remove nil?
             [entity
              (entity-by-name conn (:external-id entity))
              (entity-by-external-id conn (:name entity))
              (when id-name (entity-by-name conn id-name))
              (entity-by-id-string conn name-alias)]))))

(defn- enrich-neighbor-entity [conn entity]
  (let [name (:entity/name entity)
        alias-id (when (and name (fid/uuid-string? name))
                   (UUID/fromString name))
        alias-entity (when alias-id
                       (store/fetch-entity conn {:id alias-id} {}))]
    (or alias-entity entity)))

(defn- decorate-neighbors [conn rows]
  (mapv (fn [row]
          (update row :entity #(enrich-neighbor-entity conn %)))
        rows))

(defn- dedupe-neighbors [rows]
  (let [seen (volatile! #{})]
    (reduce (fn [acc {:keys [relation direction entity] :as row}]
              (let [neighbor-id (:entity/id entity)
                    key [relation direction neighbor-id]]
...
(defn- merged-neighbors [conn entities]
  (let [ids (keep :id entities)
        rows (mapcat #(gm/neighbors conn %) ids)]
    (vec (dedupe-neighbors rows))))

(defn- link-type? [t]
  (let [t (when (keyword? t) t)
        ns (some-> t namespace)]
    (or (= :link t)
        (= "link" ns))))

(defn- hx-events [store]
  (try
    (some-> (arxana/fulltext-dump store) :events)
    (catch Exception _ nil)))

(defn- hx-recency [hx]
  (long (or (:hx/last-seen hx)
            (:hx/recorded-at hx)
            0)))

(defn- hx-article-name [{:article/keys [name ident]}]
  (or name ident (some-> ident str) "?"))

(defn- ->hx-type [value]
  (cond
    (keyword? value) value
    (sequential? value) (some-> value first ->hx-type)
    :else nil))

(defn- normalize-hx-end [idx {:keys [role article] :as end}]
  (let [article (or article (:article end))
        ident (:article/ident article)
        type (->hx-type (:article/type article))
        name (hx-article-name article)]
    (cond-> {:idx idx
             :role (or role :participant)
             :name name
             :ident ident}
      type (assoc :type type)
      article (assoc :article article))))

(defn- decorate-hx-ends [ends]
  (let [with-idx (vec (map-indexed normalize-hx-end ends))
        source (or (some #(when (= :source (:role %)) %) with-idx)
                   (first with-idx))
        target (or (some #(when (= :target (:role %)) %) with-idx)
                   (some #(when (and source (not= (:idx %) (:idx source))) %) with-idx)
                   (second with-idx)
                   source)
        primary-idxs (set (keep :idx (remove nil? [source target])))]
    {:source source
     :target target
     :ends   (mapv (fn [end]
                     (assoc end :primary? (contains? primary-idxs (:idx end))))
                   with-idx)}))

(defn- hx-event->tail-row [hx]
  (let [{:keys [source target ends]} (decorate-hx-ends (:hx/ends hx))
        base {:hx/id (:hx/id hx)
              :hx/type (:hx/type hx)
              :hx/ends (vec ends)
              :hx/labels (:hx/labels hx)
              :hx/content (:hx/content hx)
              :confidence (or (:hx/confidence hx) (:confidence hx))
              :last-seen (or (:hx/last-seen hx) (:last-seen hx))
              :type (:hx/type hx)}]
    (cond-> base
      source (assoc :src (select-keys source [:name :type :ident]))
      target (assoc :dst (select-keys target [:name :type :ident])))))

(defn- tail-from-hx [store limit]
  (let [limit (max 1 (or limit 5))]
    (->> (or (hx-events store) [])
         (filter #(link-type? (:hx/type %)))
         (sort-by hx-recency >)
         (map hx-event->tail-row)
         (take limit)
         vec)))

(defn- tail-from-conn [conn limit]
  (try
...
(defn tail
  ...
  ([ctx]
   (tail ctx 5))
  ([ctx limit]
   (let [{:keys [conn arxana-store]} (if (map? ctx)
                                       {:conn (:conn ctx)
                                        :arxana-store (:arxana-store ctx)}
                                       {:conn ctx})]
     (cond
       (arxana-store? arxana-store) (tail-from-hx arxana-store limit)
       conn (tail-from-conn conn limit)
       :else []))))

(defn trails
  "Return the most recent trail entries from the Datascript store."
  ([ctx]
   (trails ctx 10))
  ([ctx limit]
   (let [conn (if (map? ctx) (:conn ctx) ctx)
         limit (or limit 10)]
     (if conn
       (try
         (store/recent-trails conn limit)
         (catch Exception _ []))
       []))))

(defn ego
  "Return outgoing and incoming relations for the named entity.
  Resolves the request against entity names, external ids, canonical UUIDs,
  and any aliases chained through those identifiers so scholia recorded on
  either form remain visible. Returns nil when the entity is absent."
  [conn name]
  (when-let [entity (primary-entity conn name)]
    (let [initial (related-entities conn entity)
          alias-set (distinct-entities-by-id
                     (mapcat #(related-entities conn %) initial))
          neighbors (merged-neighbors conn alias-set)
          outgoing (->> neighbors
                        (filter #(= :out (:direction %)))
                        vec
                        (decorate-neighbors conn))
          incoming (->> neighbors
                        (filter #(= :in (:direction %)))
                        vec
                        (decorate-neighbors conn))
          links {:outgoing outgoing
                 :incoming incoming
                 :all (vec (concat outgoing incoming))}]
      {:entity entity
       :outgoing outgoing
       :incoming incoming
       :links links})))
...
(defn fetch-entity
  "Fetch an entity (optionally at a historical version)."
  [{:keys [conn]} entity-spec opts]
  (store/fetch-entity conn (entity-ref entity-spec) opts))

(defn entity-history
  "Return version history for the provided entity reference."
  [{:keys [conn]} entity-spec opts]
  (store/entity-history conn (entity-ref entity-spec) opts))

(defn- normalize-entity-type [value]
  (cond
    (keyword? value) value
    (string? value) (let [trimmed (str/trim value)
                          clean (if (and (seq trimmed)
                                          (= ":" (subs trimmed 0 1)))
                                  (subs trimmed 1)
                                trimmed)]
...
(defn latest-entities
  "Return the most recently seen entities of the requested type."
  [{:keys [conn]} {:keys [type limit]}]
  (let [entity-type (normalize-entity-type type)]
    (when entity-type
      (store/latest-entities-by-type conn entity-type limit))))
