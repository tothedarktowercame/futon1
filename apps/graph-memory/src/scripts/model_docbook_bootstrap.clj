(ns scripts.model-docbook-bootstrap
  "Insert or update the docbook model descriptor in the graph store."
  (:require [app.model-docbook :as model]
            [app.store-manager :as store-manager]))

(defn- usage []
  (str "Usage: clojure -M -m scripts.model-docbook-bootstrap [--force]\n"))
  ...
  (let [opts (parse-args args)]
    ...
    (let [profile (store-manager/default-profile)
      ...
      (try
        ...
        (println "Docbook model descriptor ready.")
        (finally
