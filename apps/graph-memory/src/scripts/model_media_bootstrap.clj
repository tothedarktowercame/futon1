(ns scripts.model-media-bootstrap
  "Insert or update the media model descriptor in the graph store."
  (:require [app.model-media :as model]
            [app.store-manager :as store-manager]))

(defn- usage []
  (str "Usage: clojure -M -m scripts.model-media-bootstrap [--force]\n"))
  ...
  (let [opts (parse-args args)]
    ...
    (let [profile (store-manager/default-profile)
      ...
      (try
        ...
        (println "Media model descriptor ready.")
        (finally
