(ns scripts.model-open-world-bootstrap
  "Insert or update the open-world ingest model descriptor in the graph store."
  (:require [app.model-open-world :as model]
            [app.store-manager :as store-manager]))

(defn- usage []
  (str "Usage: clojure -M -m scripts.model-open-world-bootstrap [--force]\n"))
  ...
  (let [opts (parse-args args)]
    ...
    (let [profile (store-manager/default-profile)
      ...
      (try
        ...
        (println "Open-world ingest model descriptor ready.")
        (finally
